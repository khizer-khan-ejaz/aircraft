    ...
    def generate_question_with_reference(self, specified_reference, num_airports):
        ...
        while attempts < max_attempts:
            ...
            try:
                ...
                tas_normal = round(240 + random.random() * 10, 1)
                tas_single_engine = round(180 + random.random() * 20, 1)
                
                track = self.get_track_angle(dep, arr)
                wind_dir_normal = int((track + 180 + (random.random() - 0.5) * 60) % 360)
                wind_speed_normal = int(40 + random.random() * 30)
                wind_dir_single = int((wind_dir_normal + (random.random() - 0.5) * 20) % 360)
                wind_speed_single = int(wind_speed_normal * (0.8 + random.random() * 0.4))
